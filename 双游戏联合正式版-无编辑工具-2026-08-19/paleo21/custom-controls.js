...
(function setupPaleoCustomControls() {
  ...
  function setArRecognitionState(arena, recognized) {
    ...
    const force = arena.querySelector("[data-ar-force-recognition]");
    if (force) {
      force.setAttribute("aria-pressed", String(recognized));
      force.textContent = recognized ? "调试：取消识别" : "调试：强制识别成功";
    }
    setArBoardStatus(
      arena,
  ...
  function configureGameUi() {
    ...
    document.querySelectorAll(".header-tools button").forEach((button) => {
      if (button.textContent.trim() === "编辑布局") button.classList.add("is-removed-control");
    });
    ...
    document.querySelectorAll(".combined-arena").forEach((arena, index) => {
      ...
      const forceRecognition = document.createElement("button");
      forceRecognition.type = "button";
      forceRecognition.className = "ar-force-recognition";
      forceRecognition.dataset.arForceRecognition = "true";
      forceRecognition.setAttribute("aria-pressed", "false");
      forceRecognition.textContent = "调试：强制识别成功";
      forceRecognition.hidden = true;
      forceRecognition.addEventListener("click", () => {
        setArRecognitionState(arena, !arena.classList.contains("ar-board-recognized"));
      });
      emptyState.append(video, status, panel, readyPanel, forceRecognition);
